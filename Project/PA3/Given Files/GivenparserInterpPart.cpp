...
bool CompoundStmt(istream &in, int &line)
{
    ...
    while (runLoop)
    {
        ...
        if (Stmt(in, line) == false)
        {
            ...
            // send syntax error in CompoundStmt
            return false;
        }
...
// Factor(in, line, num, currentVal1);
    bool goodFactor = Factor(in, line, currentVal1, num);
...
bool IfStmt(istream &in, int &line)
{
    ...
    if (!LogANDExpr(in, line, currentVal))
    {
        // exit
        return false;
    }
    ...
    {
        ParseError(line, "Missing THEN after LogANDExpr in IfStmt");
        // print error no right parenth
        return false;
    }
...
bool LogANDExpr(istream &in, int &line, Value &retVal)
{
    ...
    if (Expr(in, line, currentVal1) == false)
    {
        ParseError(line, "left expr wrong in LogANDExpr");
    ...
    {
        ParseError(line, "Operator not correct in LogANDExpr");
